...
<?php
$this->assign('title', __('Data Detail Pengguna'));
...
<div class="view card card-primary card-outline">
    <div class="card-header d-sm-flex" style="background-color:#4793AF;">
        <h2 class="card-title" style="font-size:25px; color:white"><?= h($petuga->nama) ?></h2>
    </div>
    <div class="card-body table-responsive p-0">
        <table class="table table-hover text-nowrap">
            <tr>
                <th><?= __('Nomor Anggota') ?></th>
                <td><?= $this->Number->format($petuga->id) ?></td>
            </tr>
            <tr>
                <th><?= __('NIK Pengguna') ?></th>
                <td><?= h($petuga->nik) ?></td>
            </tr>
            <tr>
